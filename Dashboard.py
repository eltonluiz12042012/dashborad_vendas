...
###Tabelas aba2
quantidade_vendas_estados = dados.groupby('Local da compra')[['Local da compra']].count()
...
with aba1:
    ...
    with coluna1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'), help='Total de vendas')
        st.plotly_chart(fig_mapa_receita, use_containe_width = True)
        st.plotly_chart(fig_receitas_estados, use_containe_width = True)
    with coluna2:    
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]), help='Total de quantidade de itens vendidos')
        st.plotly_chart(fig_receita_mensal, use_containe_width = True)
        st.plotly_chart(fig_receita_categorias,  use_containe_width = True)
with aba2:
    coluna1, coluna2 = st.columns(2)
    with coluna1:
